...
from PySide6.QtWidgets import QCheckBox, QDoubleSpinBox, QHBoxLayout, QLabel, QPushButton, QSlider
...
def test_widgets_and_initial_controller_state(panel: SpeedPanel) -> None:
    ...
    assert pitch.text() == "ピッチ維持"
    assert panel.findChild(QLabel, "pitchModeLabel") is None
    assert not any(
        button.objectName().startswith("ratePreset") for button in panel.findChildren(QPushButton)
    )

    layout = panel.layout()
    assert isinstance(layout, QHBoxLayout)
    assert layout.indexOf(rate_slider) < layout.indexOf(rate_spin_box)
    assert layout.indexOf(rate_spin_box) < layout.indexOf(reset)
    assert layout.indexOf(reset) < layout.indexOf(pitch)
    assert layout.indexOf(pitch) == layout.count() - 1
...
def test_repeated_reset_at_default_is_no_op(
    panel: SpeedPanel, controller: PlaybackController, backend: FakePlaybackBackend
) -> None:
    """既定値でのresetはBackend呼出を増やさない。"""
    reset = panel.findChild(QPushButton, "resetPlaybackRateButton")
    assert reset is not None
