...
from sdp.core.playback.qt_backend import (
    ERROR_CODE_MAP,
    ERROR_MESSAGES,
    MEDIA_STATUS_MAP,
    PLAYBACK_STATE_MAP,
...
def test_playback_state_map_covers_all_qt_values() -> None:
    ...
    assert set(PLAYBACK_STATE_MAP) == set(QMediaPlayer.PlaybackState)


def test_media_status_map_covers_all_qt_values() -> None:
    """Qt の MediaStatus 全値が明示的に写像され、既定値へ丸めていない。"""
    assert set(MEDIA_STATUS_MAP) == set(QMediaPlayer.MediaStatus)
    assert set(MEDIA_STATUS_MAP.values()) == set(MediaStatus)


def test_error_map_covers_all_qt_errors_except_no_error() -> None:
    """NoError を除く Qt の全エラー値が明示的に写像されている。"""
    assert set(ERROR_CODE_MAP) == set(QMediaPlayer.Error) - {QMediaPlayer.Error.NoError}
    assert QMediaPlayer.Error.NoError not in ERROR_CODE_MAP
    assert PlaybackErrorCode.UNKNOWN_ERROR not in ERROR_CODE_MAP.values()


def test_every_reported_error_code_has_a_japanese_message() -> None:
    """Backend が通知しうる全コードにユーザー向けメッセージがある。"""
    reported = set(ERROR_CODE_MAP.values()) | {PlaybackErrorCode.UNKNOWN_ERROR}
    assert reported <= set(ERROR_MESSAGES)
