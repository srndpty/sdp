...
および Model の D&D。プレイリストからの逐次再生・現在曲・前後曲はP2-C1で実装済み。
リピート・シャッフル・メタデータは未実装（P2-C2 / P2-D）。
...
- **履歴と cursor**: 実際に通った entry_id の列と、現在位置を指す cursor を持つ。
  ...
  未来履歴が残っている状態で別 entry を直接再生すると、未来履歴を切り捨てて
  末尾へ追加する（現在 entry と同じなら重複追加しない）。
