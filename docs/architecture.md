...
- タイトルバー相当の帯（上端24px・幅80px以上）がどれかの画面と重なっていれば位置を保つ。
- どの画面とも重ならない（モニターを外した・解像度が変わった）場合は
  **primary screenの中央**へ戻す。
- サイズはprimary screenのavailable size以内へclampし、`minimumSizeHint()` を下回らせない。
...
- 読み込み時に存在確認をせず、**利用時**に存在しなければ空文字（Qtの既定動作）へ
  fallbackする（外付け・ネットワークドライブが後で戻ることがあるため）。
