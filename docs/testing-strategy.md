...
| `PlayerControls` | **FakeBackend + 実 PlaybackController** で、状態ごとのボタン活性、フォントfallbackに依存しない白いQIconと操作部の1行配置、リピート3状態の異なるアイコンとテキスト代替、シーク（ドラッグ中の非同期更新の抑止、有効なpress/releaseでの1回だけのseek、source・duration変更による古い操作の取消）、音量・ミュートの往復とフィードバックループの不在を検証する。子ウィジェットは `objectName` で取得する |
