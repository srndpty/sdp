...
| `PlayerControls` | **FakeBackend + 実 PlaybackController** で、状態ごとのボタン活性、フォントfallbackに依存しない白い自作SVGアイコンと操作部の1行配置、リピートのOFF／ALLをチェック背景・ONEを中央の1で区別することとテキスト代替、シーク（ドラッグ中の非同期更新の抑止、有効なpress/releaseでの1回だけのseek、source・duration変更による古い操作の取消）、音量・ミュートの往復とフィードバックループの不在を検証する。子ウィジェットは `objectName` で取得する |
