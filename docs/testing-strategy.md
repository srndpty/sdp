...
| `SpectrumPanel` | ControllerとPcmTapだけを受け取ること、Widgetが1つ、sourceなし／load直後／PAUSED／STOPPED／NO_MEDIAでタイマー停止、PLAYINGで開始、pausedでの最終フレーム保持とFFT停止、stop／source変更でのフレームreset、sample rate変更でのprocessor reset、1tickでsnapshot1回・FFT最大1回、Widgetのフレーム更新、PCM到着前はFFTしないこと、hidden／最小化での停止と再表示／復帰での再開、再入防止、停止後の古いtimeoutの安全性、FFT失敗で再生を変更せずControllerへ何も要求しないこと、失敗後もsource変更で復帰できること、波形解析・PlaylistModel・cacheを参照しないこと。固定sleepを使わず状態変化とqtbotの待機で判定する |
...
| 可視化ウィジェットのライフサイクル | 表示 ON/OFF と最小化でタイマーが停止すること（SPEC-04） |
