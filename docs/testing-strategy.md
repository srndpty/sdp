...
- 440Hz 正弦波、スイープ、無音。各 2 秒、対象 6 形式すべて。
...
| `VisualizersPanel` | 5種のWidgetを1つのPanelが所有すること、PLAYINGでのみタイマーが動くこと、1つのsnapshotから5種すべてを更新すること、個別非表示で対象の解析だけが止まること、全OFFでタイマーが止まってもPCMタップは受信を続けること、最小化での停止と復帰、1種の失敗で他が継続すること、**OFF→ONで失敗した可視化を再試行して復旧すること**、**snapshotの一時的な失敗では止まらず再試行すること・失敗が続いた場合だけ全可視化を失敗表示にすること**、スペクトログラムとクロマグラムが1tickで同じFFT結果を使うこと、両方非表示ならFFTを実行しないこと、shutdown後は再開しないことを検証する |
| 追加可視化性能 | 5種すべてONでもtickごとのrFFTが1回を超えないこと、スペクトログラムの履歴コピーが1tickにつき1回であること、幅・高さを変えても描画セル数と点数が有界であること、1列追加＋間引きの所要時間の記録、5種ON時も別QTimerのGUI heartbeatが進むこと（**CIへ厳格な時間上限は設けない**） |
...
- pywinautoで配布版exeを起動し、日本語・空白入りpath、主要control、設定dialogの
  Cancel／Apply、再生設定・表示設定一式の再起動復元、再生／pause／resume／mute／stop、
  相対pathの単一instance転送、最小化からの復元、ログ生成、正常終了を
...
§6.1〜§6.18のチェックボックスは各マイルストーンで得た受け入れ記録であり、
**通常変更やリリースのたびに未完了項目を上からすべて実施しない**。
変更内容に応じ、次の最小ゲートだけを使う。

| 契機 | 自動実行 | 手動確認 |
|---|---|---|
| 通常変更 | `scripts/fix.ps1` → `scripts/check.ps1` | なし |
| 再生Backend・Qt更新 | 上記 + `uv run pytest -m audio` | §7の可聴音だけ |
| UI・設定・単一instance変更 | 上記 + build + `scripts/package-gui-smoke.ps1` | UIAで判定不能な場合だけ |
| 配布候補 | release build + package GUI smoke | §7から該当する環境依存項目だけ |
| installer変更 | installer contract + installer smoke | §7のinstaller項目だけ |

自動テストですでに網羅する状態遷移をGUIで全組合せ再実行しない。GUI E2Eは
ロジックを画面まで正しく配線できている代表経路に限定し、全組合せは単体・Qt統合・
audio testを証跡とする。
...
- [x] 音量変更とミュート
- [x] 最後まで再生して「再生終了」が表示される
- [x] MP3 を開いて再生できる
- [x] 日本語・空白を含むパスを開いて再生できる
- [x] 壊れたファイル・音声でないファイルを選んでもクラッシュせず、エラーが表示される
- [x] ウィンドウを閉じて正常終了する
- [x] `%LOCALAPPDATA%\sdp\logs\sdp.log` が生成される

2026-08-01 に `uv run pytest -m audio -q`（37件）と配布版 `dist/sdp/sdp.exe` の
computer-use 操作で確認した。配布版はミュートして日本語・空白入りWAVを開き、実再生、
pause／resume、seek、終端、壊れたMP3のエラー表示、通常終了後の残processなしを確認した。
MP3は実 `QMediaPlayer` のaudio testで再生とPCM受信を確認した。可聴音質は評価していない。
...
- [x] sourceなしでプレースホルダーが出る
- [x] 再生開始でスペクトラムが動く
- [x] 一時停止で静止し、再開で動き出す
- [x] 停止でスペクトラムが消える（無音表示へ戻る）
- [x] WAV と MP3 の両方で動く
- [x] 日本語・空白を含むパスで動く
...
- [x] pauseで静止する（Peak holdも下がらない）
- [x] stopで消える（プレースホルダーへ戻る）
- [x] 曲切替で前曲のPeak holdが残らない
- [ ] mono音源でL／Rが同じ振れ方になる
- [x] stereo音源（`sine440`はL=0.5 / R=0.25）で左右差が見える
...
- [x] 音量0でもメーターが振れる
- [x] ミュート中でもメーターが振れる
- [x] varispeed（pitch補正OFF）でも取得PCM自体の振幅を表示する
- [ ] READMEの説明と画面の挙動が一致する

レイアウト:

- [x] 波形・スペクトラム・レベルメーター・プレイリストが同時に見える
- [x] プレイリストが極端に狭くならない
...
- [x] Applyしていない編集はEscで破棄される
- [x] 波形・スペクトラム・レベルメーターを個別にON/OFFできる
- [x] 3つすべてOFFでもプレイリストと再生操作が崩れない
- [ ] 3つすべてONへ戻すと、現在再生中の曲へ追従して表示が再開する
- [ ] 非表示にするとCPU負荷が下がる（タスクマネージャで目視）
- [x] 再起動後に設定した速度が復元される
- [x] 再起動後にピッチ・表示ON/OFFが復元される
...
- [x] 相対pathを受け取れる
- [x] 起動済み配布版へ別processから転送でき、2つ目のprocessが残らない
- [x] 最小化したprimaryが転送要求で復帰する
- [x] 最大化を維持したまま転送要求を受けられる
- [x] 波形・スペクトラム・レベル、速度・ピッチを実音で確認できる
...
- [x] 波形解析が完了し、再生位置に追従する
- [x] Spectrumが実音に反応する
- [x] Peak／RMSが左右別に反応する
- [x] pauseで更新が止まり、source変更で旧表示が残らない
...
通常変更では手動確認を要求しない。次は該当領域を変更したとき、または外部配布候補を
作るときだけ実施する。同じ項目を§6のマイルストーン記録と重ねて実施しない。

### 再生Backend・Qt Multimediaを変更したとき

- [ ] 最初に音量を下げ、WAV／代表的な圧縮形式を1曲ずつ可聴再生する
- [ ] 速度0.5／2.0倍とpitch補正ON／OFFで、主観的に破綻した音が出ない
- [ ] 音声デバイス切断を扱うコードを変更した場合だけ、再生中に切断してクラッシュしない

### UI・レイアウトを変更したとき

- [ ] 変更した画面だけを100%と150% DPIで見て、文字切れと操作不能がない
- [ ] Explorerからの実D&D経路を変更した場合だけ、複数ファイルの順序を1回確認する

### installer・関連付け・外部配布を変更したとき

- [ ] Explorerの「プログラムから開く」と関連付け起動を代表1形式で確認する
- [ ] 既定のアプリ設定への導線を変更した場合だけ、その導線を確認する
- [ ] 署名・配布経路を変更した場合だけ、MOTW付き成果物のSmartScreen表示を記録する
- [ ] 対応環境を表明する外部リリースだけ、Windows Sandboxまたは新規ユーザーで確認する

単一instance転送、最小化復帰、日本語・空白path、アンインストール後のregistry、
install直後の初回起動は自動smokeの対象なので、ここでは繰り返さない。
