...
- **進行状況**: P6-AとP6-Bは実装・自動テスト済み。P6-Aで設定schemaをversion 2へ拡張して
  ...
  **P6-C（UX仕上げ、設定・UI状態・プレイリストの破損／保存失敗時の統合確認、
  音量・リピート・シャッフル・現在曲・再生位置・プレイリスト選択行を復元するかの判断）が
  残っている。**
- **受け入れ条件**: 速度・ピッチ補正・可視化の表示状態が設定画面から変更でき、
  ...
  音量・リピート・シャッフル・前回の曲と再生位置、プレイリスト選択行、
  キャッシュ上限の設定UIはP6-Cで採否を判断する。
