...
| 設定schema | JSON、schema version 2（v1も読み込み可） | v1には可視化設定が無いため表示ONで補い、次の変更時にv2で保存する。起動しただけでは書き換えない |
...
- **進行状況**: P6-Aは実装・自動テスト済み。設定schemaをversion 2へ拡張して
  ...
  （[testing-strategy.md §6.12](./testing-strategy.md)）。
  **P6-B（ウィンドウ位置・サイズ、前回フォルダーなどの復元）と
  P6-C（UX仕上げ、設定破損・保存失敗時の統合確認）が残っている。**
- **受け入れ条件**: 速度・ピッチ補正・可視化の表示状態が設定画面から変更でき、
  次回起動時に復元される。可視化を非表示にすると、その可視化固有の解析・描画も止まり、
  共有PCMタップは止めない。デバウンス保存により異常終了後も直近の状態が残る。
  ウィンドウジオメトリ・前回フォルダー等の復元はP6-Bで扱う。
  音量・リピート・シャッフル・前回の曲と再生位置、キャッシュ上限の設定UIは
  P6-C以降で採否を判断する。
