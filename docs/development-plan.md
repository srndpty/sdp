...
| SPEC-04 | 非表示時・最小化時は解析と描画を停止する | P5 |
...
- **受け入れ条件**: 音量・速度・ピッチ補正・リピート・シャッフル・ウィンドウジオメトリ・
  可視化の表示状態・前回の曲と再生位置が復元される。デバウンス保存により
  異常終了後も直近の状態が残る。設定ダイアログからキャッシュ上限等を変更できる。
- **テスト**: 設定の往復とマイグレーション、復元シナリオの Qt テスト。
