...
| PLAY-11 | タイトル・サイズ・ビットレート等の音声情報表示（取得失敗時はファイル名） | P2 |
...
| VIS-02 | オシロスコープ | 低 | 低 | 中 | P8趣味機能として実装済み |
| VIS-03 | スペクトログラム | 中 | 中 | 高 | P8趣味機能として実装済み |
| VIS-04 | ステレオ・ベクトルスコープ／位相相関メーター | 中 | 低 | 高 | P8趣味機能として実装済み |
| VIS-05 | クロマグラム | 高 | 中 | 中 | P8趣味機能として実装済み |

VIS-01 と VIS-02 はスペクトラムのために作る PCM 供給基盤をそのまま使えるため、
追加コストが小さい。VIS-03 以降は表示バッファやピッチ解析の追加実装が必要になるため
MVP にも初回完成版にも含めない。

P8趣味機能では、既存の NumPy + QPainter 方針を維持し、追加依存なしで
VIS-02〜VIS-05 を実装した。ステレオ位相系は L／R のベクトルスコープに加えて、
モノ互換の判断に使える位相相関メーターも併設する。
...
- **受け入れ条件**: スライダーと数値入力で 0.5〜2.0 倍に変更でき、
...
- 追加ビジュアライザー（VIS-02 以降）の採否 — P8趣味機能で一旦すべて実装し、
  実画面で確認したうえで残すものを判断する。
