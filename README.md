...
**P1（単曲再生基盤）まで完了。1 曲を開いて再生するところまで動作する。**
...
**未実装（P2 以降）**: プレイリスト、ドラッグ＆ドロップ、前の曲 / 次の曲、
リピート・シャッフル、メタデータ表示、再生速度とピッチ補正の操作 UI、
追従波形、スペクトラム、レベルメーター、設定の永続化、
コマンドライン引数でのファイル指定、Windows のファイル関連付け、インストーラー。

ログは `%LOCALAPPDATA%\sdp\logs\sdp.log` へ出力する（1MB × 5 世代）。
