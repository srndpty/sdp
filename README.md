...
- Explorerの関連付けダブルクリックまたはファイルパス付き起動では、その曲を自動再生
...
- 一覧表示（タイトル / サイズ / ビットレート / 長さ / パス）
- タグの非同期読み取り（[Mutagen](https://mutagen.readthedocs.io/) を使用）。
  追加した直後はファイル名が表示され、読み取りが終わるとタイトル等へ更新される。
  ファイルサイズは同じ背景処理で独立して取得するため、タグ解析に失敗しても表示される。
  タグが無い・読み取れない場合はファイル名のまま表示し、再生や他の操作は妨げない
- 「ファイル」メニューまたはD&Dからの複数ファイル追加（選択順を維持）
- 選択行を`Delete`でプレイリストから削除（`Ctrl+A`後なら全行）
...
編集可能なComboBoxへの入力、モーダルダイアログ表示中は操作を奪わない。
ボタンにフォーカスがあっても`Space`は常に再生／一時停止を切り替える。
