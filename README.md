...
P5-A（PCMタップとリアルタイムスペクトラム）とP5-B（Peak／RMSレベルメーター）も
実装・自動テスト済みで、**P5は実装・自動テスト済み**。
これにより当初計画上の「sdpらしい初回完成版」に到達している。

ただし **P5は実装・自動テスト済み。実画面・実音の手動受け入れは未完了。**
P3・P4-B・P5-A・P5-Bの実画面・実マウス・実音による手動受け入れは、
リリース前ゲートとして残っている。
...
- スペクトラムと同じ再生中PCM・同じ約30FPSの更新を共有する
