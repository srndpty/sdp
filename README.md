...
P5-A（PCMタップとリアルタイムスペクトラム）も実装・自動テスト済み。
**P5-AだけではP5は完了ではない**（Peak／RMSレベルメーターはP5-Bで追加する）。
P3・P4-B・P5-Aの実画面・実マウス・実音による手動受け入れは未完了で、
リリース前ゲートとして残っている。
...
- mono float32 の固定長リングバッファ（48kHzで2秒相当。sample rate判明時に作り直す）
...
- ウィジェット非表示時とウィンドウ最小化中は更新を停止し、復帰で再開する
- スペクトラムの失敗は音声再生を妨げない

スペクトラムは**再生中のPCMから生成する**。音源を独立したデコーダーで再デコードせず、
波形キャッシュとも独立している。なお `QAudioBufferOutput` が渡すのは
...
**未実装**: Peak／RMSレベルメーター（P5-B）、スペクトログラム、ウォーターフォール、
カバーアート、メタデータの編集、オンラインからの情報取得、
ショートカット編集、プリセット編集、独立したピッチシフト、波形ズーム、ステレオ別波形、
波形の表示・色設定、可視化の表示ON/OFF設定・色設定・バンド数設定・FPS設定、
OpenGL描画、設定画面、
