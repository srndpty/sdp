...
P6-A（設定画面と可視化の表示ON/OFF）とP6-B（ウィンドウ状態と前回フォルダーの復元）は
実装・自動テスト済み。実画面での手動受け入れは未完了で、リリース前ゲートとして残っている。
...
設定の検索、音量・リピート・シャッフル・現在曲・再生位置の復元、
プレイリストの選択行の復元、OpenGL描画、
...
- 設定: `settings.json`（再生速度、ピッチ補正、可視化3種類の表示ON/OFFだけ。
  schema version 2。旧version 1のファイルも読み込め、次の変更時にversion 2で保存する）
- ウィンドウ状態: `ui-state.json`（位置・サイズ・最大化・スプリッター比率・前回フォルダーだけ。
  schema version 1。削除すると既定の位置とサイズで起動する）
...
**その起動では設定を上書き保存しない**。ウィンドウ状態ファイルが壊れていた場合も
既定の位置・サイズで起動して通知し、その起動では上書きしない。
